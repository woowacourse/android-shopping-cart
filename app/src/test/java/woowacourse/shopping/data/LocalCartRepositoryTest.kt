...
class LocalCartRepositoryTest {
    ...
    @Test
    fun `등록되지 않은 상품을 등록하면 NewAdded를 반환한다`() = runTest {
    ...
    @Test
    fun `이미 등록된 상품을 등록하면 Increment를 반환한다`() = runTest {
    ...
    @Test
    fun `수량 감소 시 장바구니에 등록된 상품의 수량을 1 감소시킨다`() = runTest {
    ...
    @Test
    fun `장바구니에 등록된 상품의 수량이 1일 때 수량을 감소시키면 장바구니에서 삭제된다`() = runTest {
        val existingEntity = CartEntity(productId, 1)
        coEvery { cartDao.findById(productId) } returns existingEntity
        coEvery { cartDao.deleteOrDecrement(productId) } returns Unit
