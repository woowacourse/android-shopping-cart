...
class DefaultProductDataSourceTest {
    @Test
    fun `페이지당 상품의 수가 허용 범위 이내이다`() {
        // when
        val actualData = DefaultProductDataSource.getProducts(0, 20).size
    ...
    fun `올바른 페이지 상품들을 가져온다`() {
        // when
        val actualData = DefaultProductDataSource.getProducts(0, 5)
    ...
    fun `상품 아이디에 부합하는 상품을 가져올 수 있다`() {
        // when
        val actualData = DefaultProductDataSource.getProductById(1)
    ...
    fun `유효하지 않은 상품 아이디로 상품 정보를 가져오면 예외를 발생시킨다`() {
        assertThrows<NoSuchElementException> { DefaultProductDataSource.getProductById(0) }
    }
}
