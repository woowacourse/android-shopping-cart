...
import woowacourse.shopping.product.catalog.fixture.FakeCatalogDataSource
...
class CatalogViewModelTest {
    ...
    @Test
    fun `초기 상태 테스트`() {
        viewModel = CatalogViewModel(FakeCatalogDataSource(size = 20))
    ...
    fun `더보기 버튼을 눌렀을 때 페이지가 증가되고 상품이 로드된다`() {
        // given
        viewModel = CatalogViewModel(FakeCatalogDataSource(size = 25))
        val catalogProducts: List<CatalogItem> = (viewModel.catalogItems.value ?: emptyList())
