...
class ShoppingCartApplication : Application() {
    override fun onCreate() {
        ...
        LocalProductsDataSource.init(shoppingCartDatabase.productDao.apply { insertAllIfEmpty() })
        LocalShoppingCartDataSource.init(shoppingCartDatabase.shoppingCartDao)
        LocalRecentViewedProductsDataSource.init(shoppingCartDatabase.recentViewedProductDao)
    }

    private fun ProductDao.insertAllIfEmpty() {
        thread {
            if (loadAll().isEmpty()) {
                insertAll(
                    listOf(
    ...
    companion object {
        private const val DATABASE_NAME = "shoppingCart"
    }
}
