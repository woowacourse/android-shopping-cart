...
interface LastProductDao {
    ...
    @Query("SELECT * FROM lastProduct ORDER BY viewedAt DESC LIMIT 10")
    fun getRecent10(): List<LastProductEntity>

    @Query("SELECT * FROM lastProduct ORDER BY viewedAt DESC LIMIT 1")
    fun getLatest(): LastProductEntity?
