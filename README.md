...
- [ ] 수량 선택 컴포넌트 기능
    - [ ] `+` 버튼 클릭 시 장바구니에 상품을 1개 추가하고 수량 조절 버튼(+, -)으로 변경한다
    - [ ] 수량 조절 버튼을 통해 장바구니에 담을 상품 수를 변경할 수 있다
    - [ ] 수량이 0이 되면 다시 초기 `+` 버튼 상태로 돌아간다
- [ ] 화면 간 상태 동기화
    - [ ] 상품 목록 화면에서 변경한 수량이 장바구니 화면에 반영된다
    - [ ] 장바구니 화면에서 변경한 수량이 상품 목록 화면에 반영된다
    - [ ] 상품 상세 화면에서 변경한 수량이 장바구니 및 목록 화면에 반영된다

### 프로그래밍 요구 사항

- [ ] 수량 선택 컴포넌트 재사용: 상품 목록, 상세, 장바구니 화면 등에서 공통으로 사용할 수 있는 UI 컴포넌트로 분리한다
- [ ] Room Database 및 Entity 분리
