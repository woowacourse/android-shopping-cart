# android-shopping-cart

## step 3 / step 4  기능 구현 목록
### 이전 단계에서 빠진 부분
- [ ] 패키지 구조 변경 (고민중)
- [ ] pageSize의 0 이상에 대한 예외처리 고민
- [x] CartStateHolder 외부 조작 제한

### 기능 구현 목록
#### 도메인
##### CartProduct
- [x] CartProduct는 Product를 가진다
- [x] 개수를 가진다
- [x] 개수는 0 이상이어야 한다
- [x] 전체 금액을 계산한다

##### Cart
- [x] 전체 개수를 계산한다
- [x] 전체의 금액을 계산한다

#### UI
##### CatalogScreen
- [x] 아이템이 있다면 장바구니 옆에 개수를 보여준다
- [x] 아이템의 + 버튼을 클릭하면 개수를 증가시킨다
- [x] 아이템의 버튼을 누르면 해당 상품을 카트에 추가한다

##### ProductDetailScreen
- [x] 해당 화면에서 개수를 증가시키면 카트에 추가한다

##### CartScreen
- [x] 해당 화면에서 개수를 증가시키면 카트에 추가한다

### 리팩토링
- [x] ViewModel 적용
- [x] Cart, Catalog Room 에 적용

- ### 최근 본 상품
- [x] 최근 본 상품이 있는 경우 상품 목록 상단에서 최대 10개까지 보여준다.
- [x] 상품 상세 페이지에 마지막으로 본 상품 1개를 보여준다.
- [x] 마지막으로 본 상품을 선택해 이동한 상세 페이지에서는 마지막으로 본 상품을 보여주지 않는다.
- [x] 마지막으로 본 상품 상세 페이지에서 뒤로 가면 상품 목록으로 이동한다.

### 데이터 유지
- [x] 앱이 재시작돼도 최근 본 상품 목록을 유지한다.
- [x] 앱이 재시작돼도 장바구니 데이터를 유지한다.
- [x] 로컬 데이터 유지를 위해 Room을 사용한다.

### 네트워크
- [x] 상품 목록 HTTP Client를 구현한다.
- [x] MockWebServer를 사용하여 HTTP Client 테스트 환경을 구성한다.
- [x] MockWebServer는 테스트 소스 세트에서만 사용한다.
- [x] 네트워크 상태 변경 시스템 이벤트를 감지하여 UI에 반영한다.


## step 1 / step 2
### 기능 구현 목록
#### 도메인 
##### Product
- [x] 상품은 id를 갖는다
- [x] 이미지 uri를 문자로 갖는다
- [x] 상품은 이름과 가격을 갖는다
- [x] 가격은 0 초과 이어야 한다

##### CartProducts
- [x] 카트에 넣은 상품들의 목록을 갖는다 
- [x] 상품을 추가하거나 제거할 수 있다
- [x] 상품을 `id`로 검색 한다

##### Cart
- [x] 사용자가 선택한 상품을 갖는다
- [x] 사용자가 선택한 상품을 추가할 수 있다
- [x] 사용자가 선택한 상품을 제거할 수 있다

#### UI
##### CatalogScreen
- [x] LazyVerticalGrid를 사용한다
- [x] 카트 아이콘을 클릭하면 CartScreen으로 이동한다
- [x] 상품 목록에서 스크롤을 20개 했을 때 더보기 버튼을 눌러 추가 20개를 로드할 수 있다
  - [x] 비동기로 데이터를 가지고 온다
**ShoppingItem**
- [x] 아이템을 클릭하면 해당 Item의 DetailScreen으로 이동한다
- [x] 이미지 로딩으로 Coil을 사용한다

##### ProductDetailScreen
- [x] 카트에 담기 버튼을 클릭하면 해당 Item을 카트에 추가한다
- [x] 카트에 담았다면 상품 목록 화면으로 돌아간다
- [x] `x` 버튼을 누르면 ShoppingItemsScreen 으로 돌아간다
- [x] 이미지 로딩으로 Coil을 사용한다

##### CartScreen
- [x] 뒤로가기 버튼을 누르면 ShoppingItemsScreen 으로 돌아간다
- [x] 담긴 아이템의 개수가 5개 이하일 경우 페이지네이션 버튼이 뜨지 않는다
- [x] 담긴 아이템의 개수가 5개 초과일 경우 페이지네이션 버튼이 뜬다
- [x] 한 화면에서 최대 5개의 아이템을 스크롤하여 보여줄 수 있다
- [x] 페이지 개수를 계산한다
- [x] 맨 처음 페이지일 경우 이전 버튼을 비활성화 한다
- [x] 마지막 페이지일 경우 다음 버튼을 비활성화 한다
**CartItem**
- [x] `x` 버튼을 누르면 해당 CartItem이 Cart에서 제거된다
- [x] 이미지 로딩으로 Coil을 사용한다

### 리팩토링 목록
#### 리팩토링
- [x] 방어적 복사 이용
- [x] 상태 이용 로직 StateHolder로 이동
- [x] 아이템을 모두 지웠을 때 Activity가 종료되는 버그 수정
- [x] page, pageSize에 대한 경계값 검사 추가
- [x] Cart 관련 데이터를 CartRepository로 변경
- [x] 현재 페이지를 저장하도록 변경

#### 추가 기능
- [x] CartScreen에서 5개 이상을 담고 보여지는 아이템들을 다 지우면 이전 페이지를 보여줌
- [x] 카트에 담기 성공했다면 "장바구니에 담았습니다" Snackbar를 노출한다
  - [x] 담고 난 후 화면전환하지 않는다

### 2차 리팩토링
#### 리팩토링
- [x] 상품 전체 리스트로 받아오는 로직을 일부만 받아오는 것으로 변경
- [x] 전역적인 Cart 객체가 아닌 Intent로 데이터 전환
- [ ] 패키지 구조 변경 (고민중)
- [x] MainActivity StateHolder 적용
- [x] 이름 변경
