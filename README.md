...
- [x] 상품 목록에서 장바구니에 담을 상품의 수를 선택할 수 있다.
  - [x] 장바구니 아이템에 수량을 추가한다.
  - [x] 상품 수량은 음수일 수 없다.
- [x] + 버튼을 누르면 장바구니에 상품이 추가됨과 동시에 수량 선택 버튼이 노출된다.
- [x] 장바구니 아이템 수량에 따라 장바구니와 상품 목록 전부 반영된다.
- [x] 상품 상세 화면에서 장바구니 담을 상품 수가 선택 가능하며 장바구니에 반영된다.
- [x] Room을 활용해 DB에 상품을 저장한다.
- [x] Room을 활용해 DB에서 장바구니 아이템을 조회한다.

## 4단계 구현할 기능 목록

- [x] 조회한 상품을 room을 활용해 저장한다.
  - [x] 상품 목록 상단에서 10개까지 확인할 수 있다
  - [x] 마지막으로 본 상품 1개를 상품 상세 페이지에서 확인할 수 있다.
- [x] 마지막으로 본 상품 페이지에서 뒤로 가기를 하면 상품 목록으로 이동한다.
- [x] 상품 목록을 db가 아닌 서버(가짜)로부터 가져온다.
  - [x] 상품 목록을 제공하는 서버를 구현한다.
