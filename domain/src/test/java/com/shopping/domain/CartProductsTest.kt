...
class CartProductsTest {
    ...
    internal fun `장바구니에 아이템을 추가할 수 있다`() {
        // given
        val cartProducts = CartProducts(listOf(Product("", "", 1)))

        // when
        val actual = cartProducts.add(Product("", "", 2))
    ...
    internal fun `장바구니에 있는 아이템을 삭제할 수 있다`() {
        // given
        val cartProducts = CartProducts(listOf(Product("", "", 1), Product("", "", 2)))

        // when
        val actual = cartProducts.remove(Product("", "", 2))
        // then
